...
global scenario_filename = "scenario_table_nothing_2021_10_21.csv"
...
function main()
    ...
    include("./Quar_ABM_disease_no_R0_b015_RAT.jl")
    include("./Quar_ABM_functions_2021_09_22.jl")

    date = "2021_10_21"

    output_dirname = pwd() * "/scenarios_nothing_b015_tf_100k/" * date *"/"
      ...
      for t in 1.0:time_increment:tf
        ...
        #  evaluate_traveller_tests!(model_main)
          ...
          if mod(t, 1) == 0
            ...
            ### for RAT scenarios, test evaluation and isolation steps are moved outside of the
            ### 'daily' evaluation block, and into the 'dt' evaluation block.
            ...
            #####
            # tests are still given daily, as one per day is still standard.
         ...
         #########
         ### for RAT version - this test evaluation and isolation code is moved
         ### out of the 'daily' code block and evaluated every timestep.
         #6) test results are returned for travellers previously tested
         evaluate_traveller_tests!(model_main)
         # 7) travellers who test positive or recently became symptomatic are put into isolation
         # 8) contacts of travellers who test positive are put into extension (if they're not already there)
