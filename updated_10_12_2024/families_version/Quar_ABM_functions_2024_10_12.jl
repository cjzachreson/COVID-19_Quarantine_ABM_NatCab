...
function initialise_agents!(n_travellers, n_workers, model::AgentBasedModel, environments)
  for i in 1:(n_workers + n_travellers)
    if i <= n_workers
      ...
      model[i].worker = true
      model[i].is_adult = true
      #println(model[i])
    else
      add_agent!(environments["Arrival"], model)
      model[i].traveller = true
      # child vs. adult status is done in household assignment
      #println(model[i])
    end
  end
end
...
# now modified to apply child and adult flags.
function assign_households!(model::AgentBasedModel,
                            n_per_hh,
                            households,
  ...
  count = 0
  hh_id = 1
  households[hh_id] = household_type(hh_id, [], false)
  for i in keys(model.agents)
     if model[i].traveller
       model[i].hh_id = hh_id
       count += 1
       if count <= n_children_per_household
         model[i].is_child = true
       elseif count <= (n_children_per_household + n_adults_per_household)
         model[i].is_adult = true
       end
...
# this is an old function used in a previous version of the code, but can be useful to
# see what happens if ALL infected arrivals are forced into vaccinated state.
    ...
    for i in keys(model.agents)
        if model[i].newly_symptomatic &&
          environments_2[model[i].pos] in ["Hotel",
            ...
            model[i].detected_clinical = true

            # detection of symptoms should be treated as a second detection
            # because it can now alter the stay duration in medihotel.
            #if (!model[i].already_detected)
            ...
            #end
            # NOTE elsif changed to if in this version
            if (environments_2[model[i].pos] == "Isolation" &&
            ...
            model[i].newly_symptomatic = false
...
function evaluate_traveller_tests!(model::AgentBasedModel)
  for i in keys(model.agents)
      if model[i].traveller && !model[i].already_detected && !model[i].detected_clinical
        # NOTE: the above means that while detection of symptoms can produce a detection flag
        # if someone is already detected, a PCR test cannot produce a new detection
        # flag if someone has already been detected either by symptom expression or PCR.
          ...
          if (model[i].awaiting_test_result &&
             ...
             if model[i].test_positive
                model[i].detected = true
                model[i].detected_test = true
                #if environments_2[model[i].pos] == "MediHotel Extension" && model[i].time_remaining_in_ext_quar < 14
                #  println("agent $i in HH $(model[i].hh_id) group $(model[i].extension_group) tested positive, should extend stay in medihotel")
                #end
...
function reinitialise_worker!(agent::agent_type, pVac_W, p_asymptomatic)
  ...
  agent.detected_test = false
  agent.detected_clinical = false
...
function compute_FoI!(agent::agent_type)
  ...
  if !agent.infected
      ...
      #check if latent: 
      if agent.latent #latent 
          t_pt = agent.time_infected
          agent.beta_t = agent.beta_min #note - this gets rescaled to 0 at the end 
      #check if agent is incubating 
      elseif agent.incubating # incubating
          t_pt = agent.time_infected
          # fixed this - consider checking effects on previous results. 
          agent.beta_t = agent.beta_max * exp(agent.k_inc * t_pt)/Vmax

      # check if recovering 
      elseif agent.recovering # recovering (i.e., symptomatic period if symptomatic)
          t_pt = agent.time_recovering

          agent.beta_t = agent.beta_max * exp(agent.k_r * t_pt)
      #if none of the above, then recovered. 
      else  
          agent.beta_t = agent.beta_min #note - this gets scaled to 0 in the rescaling step. 
      end
      ...
      agent.beta_t = (agent.beta_t - agent.beta_min)/(agent.beta_max - agent.beta_min) * agent.beta_max
...
function step_infection!(agent::agent_type, time_increment)::Float64
    ...
    elseif agent.time_infected > agent.incubation_period
           ...
           if !agent.asymptomatic && !agent.expressing_symptoms
             agent.newly_symptomatic = true # this gets switched to false after symptoms are detected.

             #NOTE: check to make sure this works to extend extension
             #if environments_2[agent.pos] == "MediHotel Extension" && agent.time_remaining_in_ext_quar <= 10.1
              # println("agent $(agent.id) in HH $(agent.hh_id) group $(agent.extension_group) in medihotel just expressed symptoms - should extend stay")
             #end
...
# This function is modified extensively in this version - family groups will be split
# and moved to medi-hotel extension, depending on adult/child status of first
# case in group - if child, group is split in 2 and 2, if adult, group is split into 1 and 3
# detection of additional cases in extension results in longer stays, and test
# schedule reset.
function isolate_travellers_family!(model::AgentBasedModel,
                                    iso_duration,
                                    ext_duration,
                                    test_schedule_ext,
                                    medi_hotel_flag,
                                    home_ext_compliance_rate,
                                    test_schedule_ext_symptoms)::Tuple{Float64, Float64}

  if !medi_hotel_flag
    error("Attempting to split families without medi hotels, this is not supported! - quitting")
    # this can be changed later if needed, i.e., splitting the undetected family members into regular hotel extension.
  end

  PCR_detections = 0.0
  clinical_detections = 0.0

    # should do this by iterating through households, not by iterating through agents.
    for hh_i in keys(households)
      hh_members = households[hh_i].members
      if !households[hh_i].is_split #if household is not split
        # determine whether or not a household member is a detected case
        n_detected_children = 0
        n_detected_adults = 0
        split_flag = false
        ...
        for j in hh_members
          if model[j].detected
            ...
            if model[j].expressing_symptoms
            ...
            if model[j].is_child
              n_detected_children += 1
              split_flag = true
            elseif model[j].is_adult
              n_detected_adults += 1
              split_flag = true
            end
          end
        ...
        if split_flag
          ...
          split_acc = false
          split_aacc = false
          # conditionals for type of split
          if n_detected_children == 1 && n_detected_adults == 0
            split_c = true
          elseif n_detected_children == 2 && n_detected_adults == 0
            split_cc = true
          elseif n_detected_children == 1 && n_detected_adults == 1
            split_ac = true
          elseif n_detected_children == 2 && n_detected_adults == 1
            split_acc = true
          elseif n_detected_children == 0 && n_detected_adults == 1
            split_a = true
          elseif n_detected_children == 0 && n_detected_adults == 2
            split_aa = true
          elseif n_detected_children == 1 && n_detected_adults == 2
            split_aac = true
          elseif n_detected_children == 2 && n_detected_adults == 2
            split_aacc = true
          end

          # for the non 2 x 2 splits, g1 has the 'undetected' agents, and
          # g2 has the 'detected' agents, for the 2x2 splits detection
          # status is irrelevant exept for split_ac, where the detected adult
          # and child are grouped together

          g1_children = [] # undetected children
          g2_children = [] # detected children
          g1_adults = [] # undetected adults or detected adults accomanying undetected children
          g2_adults = [] # detected adults or undetected adults acompanying detected children

          # these configs all result in splitting into two groups with one
          # child and one adult each.
          if split_c  || split_cc || split_aac || split_aa
            #split into 2 groups of 2, both go to extension

            for j in hh_members
              if model[j].is_child
                if isempty(g1_children)
                  push!(g1_children, model[j].id)
                else
                  push!(g2_children, model[j].id)
                end
              else
                if isempty(g1_adults)
                  push!(g1_adults, model[j].id)
                else
                  push!(g2_adults, model[j].id)
                end
              end
            end
          # one detected child, one detected adult
          elseif split_ac
            for j in hh_members
              if model[j].is_child
                if model[j].detected
          ...
          # one detected adult
          elseif split_a
            # group of 3 to extension, 1 infected adult to isolation
          ...
          elseif split_acc
            # group of 3 detected into extension, single adult to extension
            #println("g2 should be of size 3")
            for j in hh_members
              if model[j].is_child
                push!(g2_children, model[j].id)
              else
          ...
          elseif split_aacc
            # all 4 are detected, all go to extension
            for j in hh_members
              if model[j].is_child
                push!(g2_children, model[j].id)
              else
          ...
          g1 = append!(g1_children, g1_adults)
          g2 = append!(g2_children, g2_adults)

          #println("size of g2: $(size(g2)[1])" )

          # unless g2 is a single adult, move both groups to extension and
          # change sign of hh_ids (this will need to be checked throughout)

          for a_id in g1
            model[a_id].extension_group = 1
          end
          for a_id in g2
            model[a_id].extension_group = 2
          end

          if size(g2)[1] == 1 # this can only happen if a single adult is in g2
            # move g2 to isolation
            # move g1 to medihotel extension
            for a_id in g1
              moved = attempt_move!(model[a_id], environments["MediHotel Extension"],g,environments, model)
              if moved
                model[a_id].p_compliance = 1.0
                model[a_id].time_until_test = copy(test_schedule_ext)
                model[a_id].quarantine_extended = true
                model[a_id].time_remaining_in_ext_quar = ext_duration
              end
            end

            for a_id in g2
              moved = attempt_move!(model[a_id], environments["Isolation"],g,environments,model)
              model[a_id].detected = false
              model[a_id].time_remaining_in_isolation = iso_duration
              model[a_id].time_until_test = ones(size(model[a_id].time_until_test)) .* Inf
            end
          else
            #move g2 to medihotel extension
            # move g1 to medihotel extension as well.
            for a_id in hh_members
              moved = attempt_move!(model[a_id], environments["MediHotel Extension"],g,environments, model)
              if moved
                model[a_id].p_compliance = 1.0
                model[a_id].time_until_test = copy(test_schedule_ext)
          ...
          households[hh_i].is_split = true
        end
      else # household is already split, assess extension status due to new PCR positive see
           # evaluate_traveller_tests() and assess_symptoms()
        #hh_members = households[hh].members
        for i in hh_members
          # extend medihotel extension for group.
          if model[i].detected && model[i].detected_test && !model[i].detected_clinical # positive PCR, presymp. or asymp.

             PCR_detections += 1.0

            if (environments_2[model[i].pos] == "MediHotel Extension" && model[i].time_remaining_in_ext_quar < ext_duration)
               #println("agent $i PCR positive - extending medihotel stay for group members in HH $(model[i].hh_id) group $(model[i].extension_group)")

              for j in hh_members
                if model[j].extension_group == model[i].extension_group
                   model[j].time_remaining_in_ext_quar = ext_duration
                   model[j].time_until_test = copy(test_schedule_ext)
                end
              end
              # after detection response, detection flags are toggled.
              #model[i].detected = false
              #model[i].already_detected = true
            end

          elseif model[i].detected &&  model[i].detected_clinical # clinical detection
            # after clinical detction, the detection flag cannot be re-toggled by a positive PCR test.
            clinical_detections += 1.0
            #println("symptoms should extend extension period by 10 days")
            if (environments_2[model[i].pos] == "MediHotel Extension" && model[i].time_remaining_in_ext_quar < iso_duration)
              #hh_members_i = households[model[i].hh_id].members
              #println("time remaining before: $(model[i].time_remaining_in_ext_quar)")
              #println("agent $i symptoms detected - extending medihotel stay for group members in HH $(model[i].hh_id) group $(model[i].extension_group)")
              for j in hh_members
                if model[j].extension_group == model[i].extension_group
                   model[j].time_remaining_in_ext_quar = iso_duration
                   model[j].time_until_test = copy(test_schedule_ext_symptoms)
                end
              end

              #println("time remaining after: $(model[i].time_remaining_in_ext_quar)")
    ...
    #reset detection flags - this is hairy because now someone can be detected multiple times...
    for i in keys(model.agents)
      # reset detection flag NOTE: make sure symptom onset extends extension.
      if model[i].traveller && model[i].detected
         model[i].detected = false
         model[i].already_detected = true
         # extension does not get extended if an already-detected infection is detected again through PCR
      end
    end
...
function isolate_travellers!(model::AgentBasedModel,
                             ...
                             medi_hotel_flag,
                             home_ext_compliance_rate,
                             quar_extension)::Tuple{Float64, Float64}
  PCR_detections = 0.0
  clinical_detections = 0.0
          ...
          if (model[i].detected && environments_2[model[i].pos] in ["Home",
              ...
              for j in hh_members
                  ...
                  if !model[j].detected
                      ...
                      if moved
                        # new
                        model[j].time_remaining_in_ext_quar = quar_extension
                        model[j].time_until_test = copy(test_schedule_ext)
...
function initialise_arrivals!(model::AgentBasedModel,
                              ...
                              test_schedule,
                              n_children_per_household,
                              n_adults_per_household)::Tuple{Float64, Float64, Float64}
...
function record_traveller(agent::agent_type)
  ...
  linelist_T_entry[:child] = convert(Int64, agent.is_child)
  linelist_T_entry[:compliance] = agent.p_compliance
  linelist_T_entry[:household] = agent.hh_id
  linelist_T_entry[:ext_group] = agent.extension_group
    ...
    for i in keys(model.agents)
        if model[i].traveller
            ...
            elseif environments_2[model[i].pos] in ["Hotel Extension",
                  ...
                  # CZ: this will require modification - medihotel extension period can now be extended past 14 days,
                  # use the system applied in the old version to isolation extension (i.e., afer symptom onset)
                  # this will apply whenever a new case is detected:
                  # + 14 days for a new asymp/presymp detect (i.e., + pcr),
                  # + 10 days for symptom onset (from a confirmed or unconfirmed case)
                  # will need different testing schedules for each of these. i.e., + 14 -> [3, 13], + 10 -> [3, 8]
                  # tests will only be given to those who have not already been detected.

                  model[i].time_in_extended_quar += time_increment
                  model[i].time_remaining_in_ext_quar -= time_increment # new in this version
                  #if model[i].time_in_extended_quar > (quar_extension - time_increment) && !model[i].detected
                  if model[i].time_remaining_in_ext_quar <= dt && !model[i].detected # now mimics syntacs used for isolation dischage below
                    model[i].discharged = true
                  end
...
function compute_transmission!(model::AgentBasedModel,
 ...
 - n_workers_present
    ...
    for i in keys(model.agents)
        ...
        if model[i].infected &&
          !model[i].latent &&
          !(environments_2[model[i].pos] in ["Arrival", "Community", "Isolation"])
            ...
            for j in keys(model.agents)
                ...
                if !model[j].infected && !model[j].recovered
                  ...
                  if environments_2[model[i].pos] == "MediHotel Extension"  &&
                     environments_2[model[j].pos] == "MediHotel Extension" &&
                     model[i].hh_id == model[j].hh_id &&
                     model[i].extension_group == model[j].extension_group

                     n_ext_group = 0.0
                     hh_members_i = households[model[i].hh_id].members
                        for k in hh_members_i
                          if model[k].extension_group == model[i].extension_group
                            n_ext_group += 1.0
                          end
                        end
                      n_ext_group -= 1.0
                      pop_fac = 1.0 / n_ext_group
                      env_fac = FoI_fac_Hotel_TT_same



                  elseif (environments_2[model[i].pos] == "Hotel" || environments_2[model[i].pos] == "Hotel Extension" ) &&
